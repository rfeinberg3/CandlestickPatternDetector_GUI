...
{
    /// <summary>
    /// Inherits from base Recognizer method to classify candlesticks as Bearish or not.
    /// </summary>
    internal class Recognizer_Bearish : Recognizer
    {
        public string name;
        public Recognizer_Bearish() : base()
        {
            name = "Bearish";
        }
        /// <summary>
        /// Sets Bearish boolean in Smartcandlestick Dictionary
        /// </summary>
        ...
        /// <summary>
        /// Sets Bearish boolean in Smartcandlestick Dictionary for all candlesticks in the candlestickList.
        /// </summary>
        /// <param name="candlestickList"></param>
        ...
        public void RecognizeEngulfing(List<SmartCandlestick> candlestickList)
        {
            for (int i = candlestickList.Count - 2; i >= 0; i--)
            {
                ...
                {
                    candlestickList[i].booleans["BearishEngulfing"] = true;
                }
                else
                {
                    candlestickList[i].booleans["BearishEngulfing"] = false;
                }
            }
        }
        ...
        public void RecognizeHarami(List<SmartCandlestick> candlestickList)
        {
            for (int i = candlestickList.Count - 2; i >= 0; i--)
            {
                if ((candlestickList[i + 1].bodyRange > candlestickList[i].bodyRange)
                    ...
                    && (Recognize(candlestickList, i)))
                {
                    candlestickList[i].booleans["BearishHarami"] = true;
                }
                else
                {
                    candlestickList[i].booleans["BearishHarami"] = false;
                }
            }
        }
